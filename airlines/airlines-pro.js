...
const lastFlights = (flights) => {
  ...
  return console.log(
    `Los últimos vuelos del día se dirigen a: ${fiveLast.join(", ")}`
  );
};
...
const addFlights = (flights) => {
  ...
  let newFlights = {
    id: id2,
    to: prompt("¿Hacia donde se dirige?"),
    from: prompt("¿De donde sale el vuelo?"),
    cost: Number(prompt("¿Cuál es su precio?")),
    scale: Boolean(prompt("¿Hace escala? true/false")),
  };
...
const adminInterface = (flights) => {
  ...
  const addOrDelete = prompt(
    `Quieres añadir o borrar un vuelo. Escribe tu respuesta para continuar`
  );
...
const userInterface = (flights) => {
  ...
  do {
    ...
    if (userQuit === false) {
      const quitMessage =
        "Esperamos que hayas encontrado lo que te interesa. Saludos";
      alert(quitMessage);
    }
  } while (userQuit === true);
  return flights;
...
const adminOrUser = () => {
  ...
  while (permissions !== "admin" && permissions !== "user") {
    permissions = prompt(
      "Lo siento. No te hemos entendido. Eres admin o user?"
    );
  }
