const nameManager = () => {
    let yourName = prompt(`Bienvenido/a.\nPara empezar el juego, porfavor introduce tu nombre`)
    while (!yourName) {
        yourName = prompt('Debes introducir un nombre para continuar')
    }
    alert(`Hola ${yourName}, a continuación te mostramos el cartón. Que la suerte te acompañe!`)
    return yourName
}
let userName = nameManager()
...
const rankingAndPoints = () => {
    ...
    for (let i in playingNumbers) {
        if (playingNumbers[i].matched === true && playingNumbers[i].number === numberTurn) {
...
const newTurn = () => {
    ...
    alert(`Así queda el cartón despúes de este turno, ${userName}:\n${bingoCard}`)
    rankingAndPoints()
    turnCounter.push(1)
