...
const gameManager = () => {
  ...
  let endRound = ''
  let counter1 = 0
  let gameOver = 'El juego ha terminado. A continuación verás cuantas letras has acertado.\nHasta pronto.'
  let timer = gameTimer()
  ...
  do {
    for (let i in questions) {
      ...
      if (questions[i].status === 0) {
        ...
        while (!userAnswer) {
          userAnswer = prompt(`Debes introducir una respuesta o 'pasapalabra' para continuar.`)
        }
        ...
        if (currentTime >= timer) {
          const endTimeMessage = 'El tiempo se ha terminado.\nA continuación te mostramos cuantas letras has acertado.'
          alert(endTimeMessage)
          return
        }
        let anotherTurn = confirm('¿Quieres jugar otro turno')
